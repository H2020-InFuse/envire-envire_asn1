...
template<class Archive> inline void save(Archive & ar, const asn1SccDepthMap & value, const unsigned int file_version){
    ...
    byte data[asn1SccDepthMap_REQUIRED_BYTES_FOR_ENCODING];
    
    int pErrCode;
    BitStream asnbitstream = envire_asn::initBitStream(data,asn1SccDepthMap_REQUIRED_BYTES_FOR_ENCODING);
    asn1SccDepthMap_Encode(&value, &asnbitstream, &pErrCode, false);

    ar << boost::serialization::make_binary_object(data, asn1SccDepthMap_REQUIRED_BYTES_FOR_ENCODING);
}
